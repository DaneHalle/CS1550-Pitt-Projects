...
/*
 * Open the .disk file in read and update mode
 */
static FILE* openDisk(void)
{
  ...
  //Check disk size
  if(fseek(file, 0, SEEK_END)||ftell(file)!=5242880){
    fclose(file);
 ...
 * Close the .disk file 
 */
static void closeDisk(FILE *file)
{
  if(fclose(file)){
    fprintf(stderr, "disk couldn't close\n");
  }
}
 ...
 */
static long* requestBlock(FILE *file, size_t num)
{
...
/*
 * Set a bit in the bitmap. 1 will indicate that the block is used
 * and 0 indicates that the block is free. 
 */
static int bitmap(FILE *file, long index, char val)
{
  ...
  char byte;
  if(fseek(file, 5242880-(index/8)-1, SEEK_SET)||fread(&byte, 1, 1, file)!=1){
      return -1;
  }
  if(val){
    byte|=(1<<index%8);
  ...
  if(fseek(file, 5242880-(index/8)-1, SEEK_SET)||fwrite(&byte, 1, 1, file)!=1){
      return -1;
  }
  return 0;
}
 ...
 */
static int save(FILE *file, long index, void *block)
{
  if(index>=(5242880/BLOCK_SIZE-((5242880-1)/(8*BLOCK_SIZE*BLOCK_SIZE)+1))){
    fprintf(stderr, "The requested block at %ld doesn't exist\n", index);
    return -1;
  ...
  if(!rootBlock){ //root doesn't exist
    ...
    if(!out){
      ...
      if(newBlockI>0){
        ...
        if(save(disk, 0, rootBlock)||bitmap(disk, newBlockI, 1)){
          out=-EIO;
        }
      }else if(newBlockI==-1){
        out=-ENOSPC;
      ...
      if(!dir){
        ...
        if(!out){
          ...
          if(newBlockI>0){
            ...
            dir->nFiles++;
            if(save(disk, dirI, dir)||bitmap(disk, newBlockI, 1)){
              out=-EIO;
            }
          }else if(newBlockI==-1){
            out=-ENOSPC;
        ...
        off_t offset, struct fuse_file_info *fi)
{
  ...
  if(!rootBlock){//root doesn't exist
    ...
    if(!out){
      out=-ENOENT;
      //Load subdirectory
      long dir_block_idx=rootBlock->directories[dirI].nStartBlock;
      cs1550_directory_entry* dir=(cs1550_directory_entry*)load(disk, dir_block_idx);
      if(!dir){
        out=-EIO;
        free(rootBlock);
        closeDisk(disk);
          ...
          if(offset>dir->files[fileI].fsize){
            ...
            if(!out){
              ...
              if(left>writeSize&&left-writeSize>MAX_DATA_IN_BLOCK-offset){
                newI=requestBlock(disk, (left-writeSize+offset-1)/MAX_DATA_IN_BLOCK);
                if(!newI){
                  out=-ENOSPC;
                }
              }
            ...
            if(!out&&total>dir->files[fileI].fsize){
              ...
              if(save(disk, dir_block_idx, dir)){
                out=-EIO;
              }
            }
          }
